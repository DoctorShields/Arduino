...
{	
	...
	if(value != last){  
		encoderBump = value > last ? 1 : -1;	// clockwise : counter-clockwise
		if(arrowSize == 2) {	// you're moving the cursor
			...
			switch(cursorPos) {
				...
				case 2:			// you're changing PID values 
					...
					lcd.print(fixSpeed(PID[pidIndex],2,"0"));
					break;
				case 3:			// you're changing the SetPoint speed
					lcd.setCursor(11,0);
	...
	ClickEncoder::Button b = encoder->getButton();
  	if (b != ClickEncoder::Open) {
  		switch (b) {
  			// Clicked - change arrow arrowSize
			// Hold - save PID values to EEPROM
			// Release - saved = false
